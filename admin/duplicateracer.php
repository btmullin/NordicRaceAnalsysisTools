...
?>
	...
	Duplicate Racer: <select name='dup_id'>
		<?php
			$mysqli = OpenRaceResultsDatabase();
			$result = $mysqli->query('SELECT * FROM Racer ORDER BY FirstName, LastName');
	...
	Primary Racer: <select name='prim_id'>
		<?php
			$mysqli = OpenRaceResultsDatabase();
			$result = $mysqli->query('SELECT * FROM Racer ORDER BY FirstName, LastName');
			while ($row = $result->fetch_array())
	...
	<input type="submit" value="Combine Racers" name="submit">
	</form>
<?php
}
